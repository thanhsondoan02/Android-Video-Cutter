...
abstract class BaseActivity(@LayoutRes protected val layoutId: Int) : AppCompatActivity(), BaseView {
    ...
    fun navigateTo(clazz: Class<out BaseActivity>, fragmentClazz: Class<out BaseFragment>, onCallback: (Intent) -> Unit = {}) {
        val intent = Intent(this, clazz)
    ...
    fun navigateTo(clazz: Class<out BaseActivity>, fragmentClazz: Class<out BaseFragment>, bundle: Bundle, onCallback: (Intent) -> Unit = {}) {
        val intent = Intent(this, clazz)
        intent.putExtra(FRAGMENT_NAME, fragmentClazz.name)
    ...
    fun replaceFragment(fragment: BaseFragment, bundle: Bundle? = null, keepToBackStack: Boolean = true, screenAnim: IScreenAnim = FadeAnim()) {
        includeFragment(fragment, bundle, getContainerId(), true, keepToBackStack, screenAnim)
    }

    fun addFragment(fragment: BaseFragment, bundle: Bundle? = null, keepToBackStack: Boolean = true, screenAnim: IScreenAnim = FadeAnim()) {
        includeFragment(fragment, bundle, getContainerId(), false, keepToBackStack, screenAnim)
    }
    ...
    fun setStatusColor(color: Int = android.R.color.black, isDarkText: Boolean = true) {
        window?.apply {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                ...
                decorView.let {
                    it.systemUiVisibility = if (!isDarkText) {
    ...
    private fun includeFragment(fragment: Fragment, bundle: Bundle?, containerId: Int, isReplace: Boolean, keepToBackStack: Boolean, screenAnim: IScreenAnim) {
        ...
        try {
            ...
            supportFragmentManager.beginTransaction().apply {
                setCustomAnimations(screenAnim.enter(), screenAnim.exit(), screenAnim.popEnter(), screenAnim.popExit())
                if (isReplace) {
